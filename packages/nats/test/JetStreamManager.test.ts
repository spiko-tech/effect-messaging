      ...
      it.effect("Should create a stream, publish messages, and consume them", () =>
        Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Manager Test Message 1"))
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Manager Test Message 2"))
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Manager Test Message 3"))
      ...
      it.effect("Should get stream info and consumer info", () =>
        Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Info Test"))
      ...
      it.effect("Should purge stream messages", () =>
        Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Purge Test 1"))
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Purge Test 2"))
            yield* client.publish(TEST_SUBJECT, new TextEncoder().encode("Purge Test 3"))
      ...
      it.effect("Should handle consumer with multiple subjects", () =>
        Effect.scoped(
          Effect.gen(function*() {
            ...
            yield* client.publish(subject1, new TextEncoder().encode("Message on subject 1"))
            yield* client.publish(subject2, new TextEncoder().encode("Message on subject 2"))

            yield* Effect.sleep("100 millis")

            const consumer = yield* client.consumers.get(TEST_STREAM, TEST_CONSUMER)
            const fetched = yield* consumer.fetch({ max_messages: 2 })
            const messages = yield* Stream.runCollect(fetched.stream)
            const messagesArray = Chunk.toArray(messages)
