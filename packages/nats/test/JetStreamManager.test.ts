...
describe("JetStreamManager", { sequential: true }, () => {
  ...
  describe("stream management and publish/consume", () => {
    layer(testJetStream)((it) => {
      it("Should create a stream, publish messages, and consume them", () =>
        Effect.scoped(
          Effect.gen(function*() {
      ...
      it("Should list streams and consumers", () =>
        Effect.scoped(
          Effect.gen(function*() {
            yield* makeTestStreamAndConsumer()
      ...
      it("Should get stream info and consumer info", () =>
        Effect.scoped(
          Effect.gen(function*() {
            yield* makeTestStreamAndConsumer()
      ...
      it("Should purge stream messages", () =>
        Effect.scoped(
          Effect.gen(function*() {
            const manager = yield* JetStreamManager.JetStreamManager
            const client = yield* JetStreamClient.JetStreamClient
      ...
      it("Should handle consumer with multiple subjects", () =>
        Effect.scoped(
          Effect.gen(function*() {
